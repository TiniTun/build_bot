...
class PromptBuilder:
    ...
    def build(self, state: "SessionState") -> str:
        ...
        layers.append(
            self._build_return_context(
                agent_id=state.agent.agent_def.id,
    ...
    def _substitute_paths(self, text: str) -> str:
        ...
        cfg: Config = self.context.config
    ...
    def _load_bootstrap_context(self) -> str:
        ...
        bootstrap_path: Path = self.context.config.workspace / "BOOTSTRAP.md"
        if bootstrap_path.exists():
            parts.append(self._substitute_paths(bootstrap_path.read_text().strip()))

        agents_path: Path = self.context.config.workspace / "AGENTS.md"
        if agents_path.exists():
            parts.append(self._substitute_paths(agents_path.read_text().strip()))

        # Dynamic cron list
        cron_list: str = self._format_cron_list()
        if cron_list:
    ...
    def _format_cron_list(self) -> str:
        ...
        crons: list[CronDef] = self.context.cron_loader.discover_crons()
        if not crons:
            return ""

        lines: list[str] = ["## Scheduled Tasks\n"]
        for cron in crons:
            lines.append(f"- **{cron.name}**: {cron.description}")
        return "\n".join(lines)

    def _build_return_context(self, agent_id: str, timestamp: datetime) -> str:
