...
class InvalidDefError(Exception):
    """Definition file exists but is invalid."""

    def __init__(self, kind: str, def_id: str, reason: str):
        super().__init__(f"Invalid {kind} definition '{def_id}': {reason}")
        self.kind = kind
        self.def_id = def_id
        self.reason = reason
...
def parse_definition(
    ...
    raw_dist = yaml.safe_load(frontmatter_text) or {}
    return parse_fn(def_id, raw_dist, body)
...
def discover_definitions(
    ...
    for def_dir in path.iterdir():
        ...
        try:
            ...
            logging.warning(f"Failed to parse {def_dir.name}: {e}")
            continue
