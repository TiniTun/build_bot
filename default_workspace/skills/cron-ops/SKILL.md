...
when_to_use:
  - The user wants to schedule, list, or delete a recurring or one-off task
  - The user asks for a reminder at a specific future time
  - The user references cron schedules, intervals, or CRON.md files
---
...
For one-off jobs, pass `run_at` to `create_cron_job`; do not manually convert local time to UTC cron fields.
...
Use the `create_cron_job` tool. Do not create CRON.md files with `bash`, `write`, or `edit`.

1. Ask what task should run and when
2. For one-off jobs, determine the exact `run_at` datetime in ISO format with timezone or UTC offset
3. For recurring jobs, determine the schedule
4. Ask which agent should run the task
5. Ask for a brief description of what the cron does
6. If the task should run only once (e.g., "remind me tomorrow"), set `one_off: true`
7. Call `create_cron_job` with the name, description, agent, prompt, and one_off fields
8. For one-off jobs, pass `run_at` instead of calculating the cron `schedule` yourself
9. For recurring jobs, pass `schedule`
